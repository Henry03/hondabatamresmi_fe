...
import '/node_modules/dropzone/dist/dropzone-min.js'
import MediaUploader from '../../components/MediaUploader';
import toast from 'react-hot-toast';

function CarDetailEditor() {
  ...
  const [variants, setVariants] = useState([{ type: '', price: 0}])
  ...
  const createCar = () => {
    ...
    tags.forEach((tag) => {
      formData.append('tags[]', tag); // backend must parse tags as an array
    });

    variants.forEach((variant, index) => {
      formData.append(`variants[${index}][name]`, variant.type);
    ...
    const toastId = toast.loading("Creating new car page...");

    axios.post('/api/v1/cars/create', formData)
      .then(response => {
    ...
    <div className='m-2 sm:m-5'>
      <div className='flex justify-between'>
        ...
        <div className='flex gap-3'>
          ...
          <button className="btn btn-primary" onClick={createCar}>Save</button>
        </div>
      </div>
      <div className="divider divider-neutral"></div>
        <div className='flex'>
          ...
          <div className='w-full mb-3'>
            ...
            <div className="">
              ...
              <textarea className={`textarea ${errors?.name && "is-invalid"}`} placeholder="Hello!!!" id="descriptionInput" value={description} onChange={(e)=>setDescription(e.target.value)}></textarea>
              {
                errors?.description &&
        ...
        <div className='mt-3'>
          ...
          <div className='textarea rounded-xl p-4 grid grid-cols-3 gap-2' id='variantField'>
            {
                ...
                <div key={index} className='rounded-lg w-full p-3 flex flex-col gap-2 relative textarea'>
                  ...
                  <div className="w-full">
                    ...
                    <input 
                      type="text" 
                      placeholder="HRV" 
                      className={`input ${errors && errors[`variants[${index}].type`] ? 'is-invalid' : ''}`}
                      id={`type-${index}`}
                      value={variant.type} 
                      onChange={(e) => handleVariantChange(index, 'type', e.target.value)}
                    />
                    {
                      errors && errors[`variants[${index}].type`] &&
                      <span className="helper-text">{errors && errors[`variants[${index}].type`][0]}</span>
                    }
                  </div>
                  <div className='w-full'>
...
export default CarDetailEditor
